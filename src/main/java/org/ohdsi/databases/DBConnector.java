...
public class DBConnector {
  ...
  public static Connection connect(
    ...
    if (dbType.equals(DbType.MYSQL)) return DBConnector.connectToMySQL(
      server,
      user,
      password
    );
    else if (dbType.equals(DbType.MSSQL)) return DBConnector.connectToMSSQL(
      server,
      domain,
      user,
      password
    );
    else if (dbType.equals(DbType.ORACLE)) return DBConnector.connectToOracle(
      server,
      domain,
      user,
      password
  ...
  public static Connection connectToPostgreSQL(
    ...
    if (!server.contains("/")) throw new RuntimeException(
      "For PostgreSQL, database name must be specified in the server field (<host>/<database>)"
    );
    try {
      Class.forName("org.postgresql.Driver");
    } catch (ClassNotFoundException e1) {
      throw new RuntimeException(
        "Cannot find JDBC driver. Make sure the file postgresql-x.x-xxxx.jdbcx.jar is in the path"
      );
    }
    ...
    try {
      ...
      throw new RuntimeException(
        "Cannot connect to DB server: " + e1.getMessage()
      );
    }
  }
  ...
  public static Connection connectToMySQL(
    ...
    try {
      ...
      throw new RuntimeException(
        "Cannot find JDBC driver. Make sure the file mysql-connector-java-x.x.xx-bin.jar is in the path"
      );
    }
  ...
  public static Connection connectToODBC(
    String server,
    String user,
    String password
  ) {
  ...
  public static Connection connectToMSSQL(
    ...
    if (user == null || user.length() == 0) { // Use Windows integrated security
      try {
        ...
        throw new RuntimeException(
          "Cannot find JDBC driver. Make sure the file sqljdbc4.jar is in the path"
        );
      }
      ...
      try {
        ...
        throw new RuntimeException(
          "Cannot find JDBC driver. Make sure the file jtds-1.3.0.jar is in the path"
        );
      }
